...
public abstract class MapIterateState<K, V> {
	...
	protected abstract KeyValuePair<K, V> visitImpl(Map<K, V> map, boolean hasExtra, 
			MapOperateInterceptor<K, V> moip,
			IterationInfo info, Object param,
			MapPredicateVisitor<K, V> predicate, List<KeyValuePair<K, V>> out);
}
